package src.java.spring.controller.CallableTest;
...
public class CallableControllerTests extends AbstractContextControllerTests {
	...
	public void responseBody() throws Exception {
		MockHttpServletRequestBuilder result = get("/async/callable/response-body");
		ResultActions results = this.mockMvc.perform(result);
		results.andExpect(request().asyncStarted());
		results.andExpect(request().asyncResult("Callable result"));
		MvcResult returns = results.andReturn();
		System.out.println(returns.getAsyncResult());
		System.out.println(request().asyncStarted());
//		MvcResult mvcResult = this.mockMvc.perform(result)
